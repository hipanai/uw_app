...
Search, read, send, and modify emails across all registered Gmail accounts.
...
Usage:
    ...
    python3 execution/gmail_unified.py --read MESSAGE_ID --account leftclick

    # Send an email
    python3 execution/gmail_unified.py --send --to "user@example.com" --subject "Hello" --body "Message here" --account leftclick

    # Reply to an email
    python3 execution/gmail_unified.py --reply MESSAGE_ID --body "Reply text" --account leftclick

    # List accounts
    python3 execution/gmail_unified.py --accounts

Options:
    --query      Gmail search query (for search/modify operations)
    --read       Read a specific email by message ID
    --send       Send a new email (requires --to, --subject, --body)
    --reply      Reply to an email by message ID (requires --body)
...
ACCOUNTS_FILE = "config/gmail_accounts.json"
...
def list_accounts_cmd():
    ...
    if not accounts:
        ...
        print("\nTo add accounts:")
        print("  python3 execution/gmail_multi_auth.py --account leftclick")
        print("  python3 execution/gmail_multi_auth.py --account nicksaraev")
        return
...
Examples:
  ...
  Only search one account:
    python3 execution/gmail_unified.py --query "is:unread" --account leftclick
        """
    )

    # Modes
    parser.add_argument("--query", "-q", help="Gmail search query")
    parser.add_argument("--read", metavar="MESSAGE_ID", help="Read a specific email by message ID")
    parser.add_argument("--send", action="store_true", help="Send a new email")
    parser.add_argument("--reply", metavar="MESSAGE_ID", help="Reply to an email by message ID")
    ...
    parser.add_argument("--email", "-e", help="Filter by email address (e.g., nick@leftclick.ai)")
    ...
    # Validate mode
    has_mode = args.query or args.read or args.send or args.reply
    if not has_mode:
        parser.print_help()
        return 1
    ...
    # Filter by email address if requested
    if args.email:
        accounts = {k: v for k, v in accounts.items() if v.get("email") == args.email}
        if not accounts:
            print(f"No matching accounts found for email: {args.email}")
            return 1

    # For send/read/reply, require a single account
    if (args.send or args.read or args.reply) and len(accounts) > 1:
        print("Error: --send, --read, and --reply require --account or --email to specify a single account")
        return 1
    ...
    if args.send:
        ...
        account_name, account_info = next(iter(accounts.items()))
        service = get_service(account_name, account_info)
        if not service:
            return 1

        if args.dry_run:
            print(f"[DRY RUN] Would send email to {args.to}")
            print(f"  Subject: {args.subject}")
            print(f"  Body: {args.body[:100]}...")
            return 0

        result = send_message(service, args.to, args.subject, args.body)
        if args.json:
    ...
    if args.read:
        ...
        result = get_message_detail(service, args.read)
        if args.json:
            print(json.dumps(result, indent=2))
        elif result.get("error"):
            print(f"Error reading email: {result.get('error')}")
            return 1
        else:
    ...
    if args.reply:
        if not args.body:
            print("Error: --reply requires --body")
            return 1
        ...
        if args.dry_run:
            print(f"[DRY RUN] Would reply to message {args.reply}")
            print(f"  Body: {args.body[:100]}...")
            return 0

        result = reply_to_message(service, args.reply, args.body)
        if args.json:
            print(json.dumps(result, indent=2))
        elif result.get("success"):
    ...
    # Handle --query mode (search/modify)
